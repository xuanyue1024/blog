...
import { extractHueFromVisibleImage } from "../utils/color-utils";
import { getDynamicThemeColorEnabled, setHue } from "../utils/setting-utils";

/** banner 图像 URL */
export let bannerSrc = "";
/** banner 的 object-position 方向 */
export let bannerPosition: "top" | "center" | "bottom" = "center";
...
const log = (...args: unknown[]) =>
	isDev && console.log("[DynamicTheme]", ...args);
const error = (...args: unknown[]) =>
	isDev && console.error("[DynamicTheme]", ...args);
 ...
 */
async function extractAndApplyTheme(bannerImg: HTMLImageElement) {
	if (!isDynamicColorEnabled() || !bannerSrc || isExtracting) {
		return;
	}

	const bannerWrapper = document.getElementById("banner-wrapper");
	const viewportWidth = bannerWrapper?.clientWidth ?? bannerImg.clientWidth;
	const viewportHeight = bannerWrapper?.clientHeight ?? bannerImg.clientHeight;

	if (!viewportWidth || !viewportHeight) {
		log("Banner viewport is not ready yet, skip extraction");
		return;
	}
	...
	try {
		...
		const hue = await extractHueFromVisibleImage(
			bannerImg,
			viewportWidth,
			viewportHeight,
			bannerPosition,
